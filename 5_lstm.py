"""
Colab-ready pure LSTM baseline for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/lstm_outputs")
...
class PureLstmClassifier(nn.Module):
    def __init__(self, num_features: int, hidden: int = 128, layers: int = 2, dropout: float = 0.2):
        super().__init__()
        self.lstm = nn.LSTM(
...
def train_classifier(train_examples, task):
    ...
    model = PureLstmClassifier(num_features=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"lstm_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_lstm",
...
def evaluate_saturation(task, model, threshold, eval_examples):
    ...
    return {**base, "selected_threshold": threshold, "binary_accuracy": base["accuracy"], "message_accuracy": base["accuracy"], "trajectory_accuracy": aggregate_accuracy(eval_examples, scores, threshold), "time_per_message_ms": ms_msg, "cnn_time_per_message_ms": ms_msg, "timesfm_feature_time_per_message_ms": 0.0, "time_per_flight_ms": ms_msg * WINDOW_BY_TASK[task], "localization_error_m": float(np.mean(errs)) if errs else float("nan"), "perfect_percent": float(np.mean(top_hits)) if top_hits else float("nan")}
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE LSTM TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_lstm"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "lstm_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE LSTM baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"LSTM params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "lstm_all_tables_metrics.csv")
