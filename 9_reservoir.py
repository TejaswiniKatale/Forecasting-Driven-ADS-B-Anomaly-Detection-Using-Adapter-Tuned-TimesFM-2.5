"""
Colab-ready pure Reservoir baseline (ESN-style) for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/reservoir_outputs")
...
class PureReservoirClassifier(nn.Module):
    """
    Fixed reservoir dynamics + trainable readout head.
    """
    ...
    def __init__(
        ...
        spectral_scale: float = 0.90,
        leak_rate: float = 0.30,
        ...
        win = torch.randn(reservoir_dim, input_dim, generator=g) * input_scale
        wres = torch.randn(reservoir_dim, reservoir_dim, generator=g) / np.sqrt(reservoir_dim)

        # Approximate spectral normalization via power iterations.
        v = torch.randn(reservoir_dim, 1, generator=g)
        for _ in range(20):
            v = wres @ v
            v = v / (v.norm() + 1e-8)
        spectral = float((v.t() @ wres @ v).abs().item())
        scale = spectral_scale / max(spectral, 1e-6)
        wres = wres * scale
        ...
        self.readout = nn.Sequential(
            nn.Linear(reservoir_dim * 2, hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, T, F]
        bsz, tlen, _ = x.shape
        h = torch.zeros(bsz, self.reservoir_dim, device=x.device)
        hs = []
        for t in range(tlen):
            u = x[:, t, :]  # [B, F]
            pre = u @ self.Win.t() + h @ self.Wres.t() + self.bias
            h_new = torch.tanh(pre)
            h = (1.0 - self.leak_rate) * h + self.leak_rate * h_new
            hs.append(h)
        h_seq = torch.stack(hs, dim=1)  # [B, T, R]
        h_last = h_seq[:, -1, :]
        h_mean = h_seq.mean(dim=1)
        feat = torch.cat([h_last, h_mean], dim=1)
...
def train_classifier(train_examples, task):
    ...
    model = PureReservoirClassifier(input_dim=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"reservoir_{task}.pt")
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    return model, threshold, {
        "training_mode_actual": "pure_reservoir",
        "timesfm_trainable_parameters": 0,
        "adapter_trainable_parameters": 0,
        "cnn_trainable_parameters": int(trainable_params),
        "total_trainable_parameters": int(trainable_params),
        "total_reported": int(total_params),
        "cnn_trainable": int(trainable_params),
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE RESERVOIR TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_reservoir"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "reservoir_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE RESERVOIR baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"Reservoir params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "reservoir_all_tables_metrics.csv")
