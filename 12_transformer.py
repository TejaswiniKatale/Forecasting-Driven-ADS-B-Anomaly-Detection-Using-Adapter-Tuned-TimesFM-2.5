"""
Colab-ready pure Transformer baseline for ADS-B anomaly detection.
Standalone script: does not import 1_cnn.py or TimesFM.
"""
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/transformer_outputs")
...
class PureTransformerClassifier(nn.Module):
    def __init__(self, num_features: int, d_model: int = 128, nhead: int = 8, num_layers: int = 4, dim_ff: int = 256, dropout: float = 0.1, max_len: int = 1024):
        super().__init__()
...
def train_classifier(train_examples, task):
    ...
    model = PureTransformerClassifier(num_features=len(MODEL_FEATURES), d_model=128, nhead=8, num_layers=4, dim_ff=256, dropout=0.1, max_len=max(256, WINDOW_BY_TASK[task] + 2)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"transformer_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_transformer",
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE TRANSFORMER TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_transformer"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "transformer_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE TRANSFORMER baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"Transformer params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "transformer_all_tables_metrics.csv")
