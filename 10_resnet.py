"""
Colab-ready pure ResNet baseline for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/resnet_outputs")
...
class ResBlock1D(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, stride: int = 1, dropout: float = 0.1):
        ...
        self.shortcut = (
            ...
            else nn.Sequential(nn.Conv1d(in_ch, out_ch, kernel_size=1, stride=stride, bias=False), nn.BatchNorm1d(out_ch))
        )
...
class PureResNetClassifier(nn.Module):
    def __init__(self, num_features: int):
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv1d(num_features, 64, kernel_size=7, stride=1, padding=3, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
        )
        self.layer1 = nn.Sequential(ResBlock1D(64, 64, stride=1), ResBlock1D(64, 64, stride=1))
        self.layer2 = nn.Sequential(ResBlock1D(64, 128, stride=2), ResBlock1D(128, 128, stride=1))
        self.layer3 = nn.Sequential(ResBlock1D(128, 192, stride=2), ResBlock1D(192, 192, stride=1))
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Sequential(nn.Flatten(), nn.Linear(192, 96), nn.ReLU(), nn.Dropout(0.2), nn.Linear(96, 1))
...
def train_classifier(train_examples, task):
    ...
    model = PureResNetClassifier(num_features=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"resnet_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_resnet",
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE RESNET TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_resnet"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "resnet_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE RESNET baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"ResNet params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "resnet_all_tables_metrics.csv")
