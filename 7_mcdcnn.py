"""
Colab-ready pure MCDCNN baseline for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/mcdcnn_outputs")
...
class _FeatureBranch(nn.Module):
    def __init__(self, out_ch: int = 32):
        ...
        self.net = nn.Sequential(
            ...
            nn.BatchNorm1d(16),
            nn.ReLU(),
            nn.Conv1d(16, out_ch, kernel_size=5, padding=2),
...
class PureMCDCNNClassifier(nn.Module):
    """
    Multi-Channel Deep CNN:
    one small 1D-CNN branch per feature, fused for classification.
    """

    def __init__(self, num_features: int, branch_dim: int = 32, hidden: int = 128, dropout: float = 0.2):
        super().__init__()
        self.num_features = num_features
        self.branches = nn.ModuleList([_FeatureBranch(out_ch=branch_dim) for _ in range(num_features)])
        self.head = nn.Sequential(
            nn.Linear(num_features * branch_dim, hidden),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, 1),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outs = []
        for i in range(f):
            xi = x[:, :, i].unsqueeze(1)  # [B,1,T]
            outs.append(self.branches[i](xi))
...
def train_classifier(train_examples, task):
    ...
    model = PureMCDCNNClassifier(num_features=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"mcdcnn_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_mcdcnn",
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE MCDCNN TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_mcdcnn"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "mcdcnn_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE MCDCNN baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"MCDCNN params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "mcdcnn_all_tables_metrics.csv")
