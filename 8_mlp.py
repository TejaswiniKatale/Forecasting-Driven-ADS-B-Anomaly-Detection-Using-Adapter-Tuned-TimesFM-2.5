"""
Colab-ready pure MLP baseline for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/mlp_outputs")
...
class PureMlpClassifier(nn.Module):
    def __init__(self, seq_len: int, num_features: int, h1: int = 512, h2: int = 256, dropout: float = 0.25):
        super().__init__()
...
def train_classifier(train_examples, task):
    ...
    model = PureMlpClassifier(seq_len=WINDOW_BY_TASK[task], num_features=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"mlp_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_mlp",
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE MLP TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_mlp"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "mlp_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE MLP baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"MLP params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "mlp_all_tables_metrics.csv")
