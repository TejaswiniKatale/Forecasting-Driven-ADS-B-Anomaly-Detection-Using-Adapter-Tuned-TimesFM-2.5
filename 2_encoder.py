"""
Colab-ready pure Encoder baseline for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/encoder_outputs")
...
class PureEncoderClassifier(nn.Module):
    def __init__(self, num_features: int, d_model: int = 128, nhead: int = 8, num_layers: int = 3, ff_dim: int = 256, dropout: float = 0.2):
        super().__init__()
        self.inp = nn.Linear(num_features, d_model)
        self.pos = PositionalEncoding(d_model=d_model, max_len=512)
        enc = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=ff_dim, dropout=dropout, activation="gelu", batch_first=True)
...
def train_classifier(train_examples, task):
    ...
    model = PureEncoderClassifier(num_features=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"encoder_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_encoder",
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE ENCODER TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_encoder"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "encoder_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE ENCODER baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"Encoder params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "encoder_all_tables_metrics.csv")
