"""
Colab-ready pure CNN baseline for ADS-B anomaly detection.
Uses the same preprocessing, attacks, split policy, and metrics layout
as the TimesFM+CNN pipeline for fair comparison.
"""
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/cnn_outputs")
...
class WindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # CNN expects (C, T)
        x = torch.from_numpy(ex.x.astype(np.float32)).transpose(0, 1)
...
def standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # If multiple original columns map to the same canonical name (e.g., time/timestamp),
    # coalesce duplicates into one column to avoid df["col"] returning a DataFrame.
    if df.columns.duplicated().any():
        merged = pd.DataFrame(index=df.index)
        for col in pd.unique(df.columns):
            block = df.loc[:, df.columns == col]
            if isinstance(block, pd.Series) or block.shape[1] == 1:
                merged[col] = block.iloc[:, 0] if isinstance(block, pd.DataFrame) else block
            else:
...
def to_seconds(series: pd.Series) -> pd.Series:
    # Be defensive: if duplicate column names slipped through, we may receive a DataFrame.
    ...
    return pd.to_datetime(series, errors="coerce").astype("int64") / 1e9
            ...
            if task == "saturation":
                for cand, label, err, role in saturation_candidates(seq, h):
                    out.append(
                        Example(
                            task=task,
                            flight_id=fid,
                            x=local_feature_transform(cand[:w], ref_lat, ref_lon, means, stds),
                            y=label,
                            future=local_feature_transform(cand[w:], ref_lat, ref_lon, means, stds),
                            raw_error_m=err,
                            case_id=sat_case,
                            role=role,
                        )
                ...
                out.append(
                    Example(
                        task=task,
                        flight_id=fid,
                        x=local_feature_transform(ctx, ref_lat, ref_lon, means, stds),
                        y=0,
                        future=local_feature_transform(fut, ref_lat, ref_lon, means, stds),
...
def print_example_sanity(task: str, tr: list[Example], ev: list[Example]) -> None:
    ...
    print(f"{task} unique train case_ids={len(set(e.case_id for e in tr))}")
    print(f"{task} unique eval case_ids={len(set(e.case_id for e in ev))}")
...
def metrics_at_threshold(y_true: np.ndarray, scores: np.ndarray, t: float) -> dict:
...
def predict_scores(model: nn.Module, examples: list[Example]) -> tuple[np.ndarray, float]:
    ...
    return scores, 1000.0 * elapsed / max(1, len(ds))


def train_classifier(train_examples: list[Example], task: str) -> tuple[nn.Module, float, dict]:
    ...
    model = PureCnnClassifier(in_channels=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"cnn_{task}.pt")

    cnn_params = sum(p.numel() for p in model.parameters())
    stats = {
        "training_mode_actual": "pure_cnn",
        "timesfm_trainable_parameters": 0,
        "adapter_trainable_parameters": 0,
        "cnn_trainable_parameters": int(cnn_params),
        "total_trainable_parameters": int(cnn_params),
        "total_reported": int(cnn_params),
        "cnn_trainable": int(cnn_params),
        "model_name_actual": MODEL_NAME,
    }
    return model, threshold, stats


def aggregate_accuracy(examples: list[Example], scores: np.ndarray, threshold: float, by: str) -> float:
...
def evaluate_binary(task: str, model: nn.Module, threshold: float, eval_examples: list[Example]) -> dict:
    ...
    return {
        **base,
        ...
        "time_per_message_ms": ms_msg,
        "cnn_time_per_message_ms": ms_msg,
        "timesfm_feature_time_per_message_ms": 0.0,
        "time_per_flight_ms": ms_msg * WINDOW_BY_TASK[task],
        "localization_error_m": float("nan"),
        "perfect_percent": float("nan"),
    }


def evaluate_saturation(task: str, model: nn.Module, threshold: float, eval_examples: list[Example]) -> dict:
    y = np.asarray([e.y for e in eval_examples], dtype=np.int64)
    scores, ms_msg = predict_scores(model, eval_examples)
    base = metrics_at_threshold(y, scores, threshold)
    groups: dict[str, list[int]] = {}
    ...
    return {
        **base,
        ...
        "message_accuracy": base["accuracy"],
        "trajectory_accuracy": aggregate_accuracy(eval_examples, scores, threshold, by="case"),
        "time_per_message_ms": ms_msg,
        "cnn_time_per_message_ms": ms_msg,
        "timesfm_feature_time_per_message_ms": 0.0,
        "time_per_flight_ms": ms_msg * WINDOW_BY_TASK[task],
...
def print_final_tables(results: dict, total_params: int) -> None:
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE CNN TABLE VALUES ONLY")
    print("=" * 78)
    spoof_df = pd.DataFrame([{"Model": MODEL_NAME, "Accuracy": pct(spoof["case_accuracy"]), "Time/msg.": ms(spoof["time_per_message_ms"]), "Parameters": f"{total_params:,}"}])
    ...
    interp_df = pd.DataFrame([{"Model": MODEL_NAME, "Acc.": pct(interp["case_accuracy"]), "Time/msg.": ms(interp["time_per_message_ms"]), "Msg. Acc.": pct(interp["message_accuracy"]), "Precision": pct(interp["precision"]), "Recall": pct(interp["recall"]), "F1": pct(interp["f1"]), "ROC-AUC": pct(interp["roc_auc"]), "FPR": pct(interp["fpr"])}])
    replay_df = pd.DataFrame([{"Model": MODEL_NAME, "Acc.": pct(replay["case_accuracy"]), "Recall": pct(replay["recall"]), "FPR": pct(replay["fpr"]), "Precision": pct(replay["precision"]), "F1": pct(replay["f1"]), "ROC-AUC": pct(replay["roc_auc"]), "Time/msg.": ms(replay["time_per_message_ms"])}])
...
def save_all_tables_metrics(results: dict, eval_windows: dict, param_stats_by_attack: dict, cleaned_usable_flights: int, total_params: int) -> None:
    rows: list[dict[str, Any]] = []

    def add(table_id: str, table_name: str, attack: str, model: str, metric_name: str, metric_value: Any, metric_unit: str = "", notes: str = "") -> None:
        rows.append(
            {
                "table_id": table_id,
    ...
    common_names = [
        ("num_train_flights", REQUIRED_TRAIN_FLIGHTS, "count"),
        ("num_eval_flights", REQUIRED_EVAL_FLIGHTS, "count"),
    ]

    for attack, table_id, table_name in [
        ("spoofing", "Table 2", "Spoofing"),
        ("saturation", "Table 3", "Saturation"),
        ("interpolation", "Table 4", "Interpolated"),
        ("replay", "Table 5", "Replay"),
    ]:
        r = results[attack]
        ps = param_stats_by_attack.get(attack, {})
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_cnn"))
        # table-specific rows
        if attack == "spoofing":
            add(table_id, table_name, attack, MODEL_NAME, "Accuracy", r.get("case_accuracy", np.nan), "ratio")
            add(table_id, table_name, attack, MODEL_NAME, "Time/msg.", r.get("time_per_message_ms", np.nan), "ms")
        ...
        for name, val, unit in common_names:
            add(table_id, table_name, attack, MODEL_NAME, name, val, unit)
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "cnn_all_tables_metrics.csv", index=False)
...
def main() -> None:
    ...
    print(f"CNN params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "cnn_all_tables_metrics.csv")
