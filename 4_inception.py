"""
Colab-ready pure InceptionTime-style baseline for ADS-B anomaly detection.
...
SEED = 42
OUTPUT_DIR = Path("/content/drive/MyDrive/Opensky_Flight_Project/inception_outputs")
...
class InceptionBlock1D(nn.Module):
    def __init__(self, in_ch: int, bottleneck: int = 32, out_ch: int = 32, kernels=(9, 19, 39)):
        super().__init__()
        self.use_bottle = in_ch > 1
        self.bottleneck = nn.Conv1d(in_ch, bottleneck, kernel_size=1, bias=False) if self.use_bottle else nn.Identity()
        bch = bottleneck if self.use_bottle else in_ch
        self.conv1 = nn.Conv1d(bch, out_ch, kernel_size=kernels[0], padding=kernels[0] // 2, bias=False)
        self.conv2 = nn.Conv1d(bch, out_ch, kernel_size=kernels[1], padding=kernels[1] // 2, bias=False)
        self.conv3 = nn.Conv1d(bch, out_ch, kernel_size=kernels[2], padding=kernels[2] // 2, bias=False)
...
class ResidualShortcut1D(nn.Module):
...
class PureInceptionClassifier(nn.Module):
    def __init__(self, num_features: int, depth: int = 6, out_ch: int = 32, dropout: float = 0.2):
        super().__init__()
        blocks = []
        in_ch = num_features
        self.shortcuts = nn.ModuleList()
        self.shortcut_points = []
        for i in range(depth):
            blk = InceptionBlock1D(in_ch, bottleneck=32, out_ch=out_ch)
            blocks.append(blk)
            new_ch = out_ch * 4
            if i % 3 == 2:
                # Residual every 3 blocks, matching InceptionTime pattern.
                prev_ch = num_features if i == 2 else out_ch * 4
                self.shortcuts.append(ResidualShortcut1D(prev_ch, new_ch))
                self.shortcut_points.append(i)
            in_ch = new_ch
        self.blocks = nn.ModuleList(blocks)
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(out_ch * 4, 96),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(96, 1),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            out = blk(x)
            x = out
            if i in self.shortcut_points:
                x = self.shortcuts[shortcut_idx](res_input, x)
                res_input = x
                shortcut_idx += 1
        x = self.gap(x)
...
def train_classifier(train_examples, task):
    ...
    model = PureInceptionClassifier(num_features=len(MODEL_FEATURES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), OUTPUT_DIR / f"inception_{task}.pt")
    params = sum(p.numel() for p in model.parameters())
    return model, threshold, {
        "training_mode_actual": "pure_inception",
...
def print_final_tables(results, total_params):
    ...
    print("\n" + "=" * 78)
    print("FINAL PURE INCEPTION TABLE VALUES ONLY")
    print("=" * 78)
...
def save_all_tables_metrics(results, eval_windows, param_stats_by_attack, cleaned_usable_flights, total_params):
    ...
    for attack, table_id, table_name in [("spoofing", "Table 2", "Spoofing"), ("saturation", "Table 3", "Saturation"), ("interpolation", "Table 4", "Interpolated"), ("replay", "Table 5", "Replay")]:
        ...
        add(table_id, table_name, attack, MODEL_NAME, "training_mode_actual", ps.get("training_mode_actual", "pure_inception"))
        if attack == "spoofing":
    ...
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "inception_all_tables_metrics.csv", index=False)


def main():
    seed_everything(SEED)
    print("Running PURE INCEPTION baseline (no TimesFM).")
    print("Device:", DEVICE)
    ...
    print(f"Inception params: {final_param_stats.get('cnn_trainable', 0):,}")
    ...
    print("Saved all tables metrics CSV:", OUTPUT_DIR / "inception_all_tables_metrics.csv")
